...
protected:
	...
	public:
		...
		size_t BATCount_;			// Number of elements in the BAT array (0x002C)//Total number of sectors used for the sector allocation table.sectors nums wcq
		size_t propertiesStart_;	// Block index of the first block of the property table (0x0030)//SecID of first sector of the directory stream.wcq
...
struct CODE
{
	enum {	FORMULA=0x0006, 		//Token array and the result of a formula cell.
			...
			STANDARDWIDTH=0x0099, 		//wcq
			FNGROUPCOUNT=0x009C, 		//wcq
			...
			INTERFACEHDR=0x00E1,	//wcq.
			INTERFACEEND=0x00E2,	//wcq.
			MERGEDCELLS=0x00E5, 	//All merged cell ranges of the current sheet.
			PHONETICPR=0x00EF, 	//wcq
			...
			DEFAULTROWHEIGHT=0x0225,//default row height.for sheet.added by wcq 2011.1.25
...
struct LargeString
{
	...
	size_t ContinueRead(const char* data, size_t size,bool bPreContinue);//added by wcq
	size_t Write(char* data);
	size_t DataSize();
	size_t RecordSize();
	size_t StringSize();
...
public:
	struct InterfaceHdr: public Record//wcq
	{
		InterfaceHdr();
		virtual size_t Read(const char* data);
		virtual size_t Write(char* data);
	...
	struct InterfaceEnd: public Record//wcq
	{
		InterfaceEnd();
		virtual size_t Read(const char* data);
		virtual size_t Write(char* data);
	...
	struct WriteAccess;//specifies the name of user who last created,opened,or modified the file.wcq
	struct FileProtection;
	struct CodePage: public Record//wcq
	{
		CodePage();
		virtual size_t Read(const char* data);
		virtual size_t Write(char* data);
	...
	struct RefreshAll;
	struct BookBool;
	struct Font : public Record//Font2
	{
		Font();
		virtual size_t Read(const char* data);
		virtual size_t Write(char* data);
	...
	BOF bof_;
	//InterfaceHdr interfaceHdr_;//wcq
	//InterfaceEnd InterfaceEnd_;//wcq
	CodePage codePage_;//wcq
	Window1 window1_;
	vector<Font> fonts_;
...
public:
	...
	struct CalculationSettings//wcq
	{
		//some order has changed by wcq. 
		struct CalcMode: public Record
		{
			CalcMode();
			virtual size_t Read(const char* data);
			virtual size_t Write(char* data);
			short fAutoRecalculate_;//must be one of following:0:Manual,1:AutoMatic,2:AutoMatic,except for tables.specifies the calculation mode for the workbook.wcq
		};
		struct CalcCount: public Record
		{
			CalcCount();
			virtual size_t Read(const char* data);
			virtual size_t Write(char* data);
			short iterationCount_;//specifies the iteration count for a calculation in iterative calculation.>=1 <=32767.wcq
		};
		struct RefMode: public Record//specifies the reference style for the workbook.
		{
			RefMode();
			virtual size_t Read(const char* data);
			virtual size_t Write(char* data);
			short fRefA1_;//a Boolean that specifies the reference style for all formulas.0:R1C1,1:A1.wcq
		};
		struct Iteration: public Record//specifies the state of iterative calculation.wcq
		{
			Iteration();
			virtual size_t Read(const char* data);
			virtual size_t Write(char* data);
			short enable_;//.a Boolean that specifies whether to enable iterative calculation.wcq
		};
		struct Delta: public Record//specifies the minimum value change required for iterative calculation to continue.wcq
		{
			Delta();
			virtual size_t Read(const char* data);
			virtual size_t Write(char* data);
			double numDelta_;//f.wcq
			private:
				union
				{
					LONGINT intvalue_;
		...
		struct SafeRecalc: public Record//specifies recalculation behavior.wcq
		{
			SafeRecalc();
			virtual size_t Read(const char* data);
			virtual size_t Write(char* data);
			short fSaveRecalc_;//.a Boolean that specifies whether to recalculate the workbook before it is saved,when in manual calculation mode.wcq
		};
		size_t Write(char* data);
		size_t DataSize();
		size_t RecordSize();
		CalcMode calcMode_;
		CalcCount calcCount_;
		RefMode refMode_;
		Iteration iteration_;
		Delta delta_;
		SafeRecalc safeRecalc_;
	};
	struct PrintHeaders;
	struct PrintGridlines;
	struct Gridset;
	struct Guts: public Record
	{
		Guts();
		virtual size_t Read(const char* data);
		virtual size_t Write(char* data);
		short unused1_;
		short unused2_;
		short rowOutlineLevel_;//specifies 
		short colOutlineLevel_;
	};
	struct DefaultRowHeight : public Record//wcq
	{
		DefaultRowHeight();
		virtual size_t Read(const char* data);
		virtual size_t Write(char* data);
		short options_;
		short noralRowHeight_;//specifies the default row height ,in twips,for empty rows.Must exist if and only if fDyZero is 0.wcq
		//short hiddenRowHeight_;//specifies the default row height ,in twips,to apply to a hidden row when unhidden.Must exist if and only if fDyZero is 0.omit here ,you can deal it???(see:http://msdn.microsoft.com/en-us/library/dd945785(v=offic.12).aspx).*this may occur errors??????wcq
	};
	struct WSBool : public Record//specifies information about a sheet.wcq
	{
		WSBool();
		virtual size_t Read(const char* data);
		virtual size_t Write(char* data);
		short options_;//include page breaks,is a dialog sheet,outline,..and so on.wcq
	};
	struct PageSettings
	{
		struct Header: public Record//wcq.the header text for printting
		{
			Header();
			virtual size_t Read(const char* data);
			virtual size_t Write(char* data);
		...
		struct Footer: public Record//wcq.the header text for printting
		{
			Footer();
			virtual size_t Read(const char* data);
			virtual size_t Write(char* data);
		...
		struct HCenter: public Record
		{
			HCenter();
			virtual size_t Read(const char* data);
			virtual size_t Write(char* data);
			short bHcenter_;//Boolean,0x0 or 0x1
		};
		struct VCenter: public Record
		{
			VCenter();
			virtual size_t Read(const char* data);
			virtual size_t Write(char* data);
			short bVcenter_;//Boolean,0x0 or 0x1
		};
		struct LeftMargin: public Record
		{//wcq
			LeftMargin();
			virtual size_t Read(const char* data);
			virtual size_t Write(char* data);
			double margin_;//<=49 and >=0
			private:
				union
				{
					LONGINT intvalue_;
		...
		struct RightMargin: public Record
		{//wcq
				RightMargin();
				virtual size_t Read(const char* data);
				virtual size_t Write(char* data);
		...
		struct TopMargin: public Record
		{//wcq
				TopMargin();
				virtual size_t Read(const char* data);
				virtual size_t Write(char* data);
		...
		struct BottomMargin: public Record
		{//wcq
				BottomMargin();
				virtual size_t Read(const char* data);
				virtual size_t Write(char* data);
		...
		struct PLS;
		struct Setup: public Record
		{
			Setup();
			virtual size_t Read(const char* data);
			virtual size_t Write(char* data);
			short papersize_;//0x77--A4
			short scale_;
			short pagestart_;//specifies the starting page number.
			short fitwidth_;//specifies the number of pages the sheet width is fit to.wcq
			short fitheight_;
			short options_;//include leftToRight,Portrait,fNoPls,fNoColor,fDraft,fNotes,fNoOrient,,fEndNotes,Errors,..and so on.wcq
			short res_;//specifies the print resolution in dots per inch(DPI),if fNoPls is 1,this value is undefined and must be ignored.wcq
			short vres_;//specifies the vertical print resolution
			double numHdr_;//specifies the header margin in inches.>=0,<=49
			double numFdr_;//specifies the footer margin in inches.>=0,<=49
			short copies_;//specifies the number of copies to print.
			private:
			union
			{
				LONGINT intvalue_;
				double doublevalue_;
			} intdoublenumHdr_,intdoublenumFdr_;
	...
	struct DefColWidth: public Record//wcq
	{
		DefColWidth();
		virtual size_t Read(const char* data);
		virtual size_t Write(char* data);
	...
	struct CellTable
	{
		struct RowBlock
		{
			struct Row : public Record
			{
				...
				virtual size_t Read(const char* data);
				virtual size_t Write(char* data);
				short rowIndex_;//specifies 
				short firstCellColIndex_;
				short lastCellColIndexPlusOne_;
				short height_;
				short unused1_;
				short unused2_;
				//int options_;//is two short == int???wcq
				short options1_;
				short options2_;
			};
			struct CellBlock
...
//				bool normalType_;
	...
	struct Pane;
	struct Selection;
	struct StandardWidth: public Record
	{
		StandardWidth();
		virtual size_t Read(const char* data);
		virtual size_t Write(char* data);
	...
	BOF bof_;
	Index index_;
	CalculationSettings calculationSettings_;//wcq
	Guts guts_;//wcq
...
//print settings.added by wcq
	void setLeftMargin(double marginLeft,size_t worksheetindex);//set LeftMargin of the worksheet for printing
private:
