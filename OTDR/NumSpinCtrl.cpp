...
double CNumSpinCtrl::GetPos()
{
	...
	if (pEdit)
	{
		...
		WideCharToMultiByte(CP_OEMCP, 0, strPath, -1, pszPath, strPath.GetLength() * 2, NULL, NULL);
		double val = atof (pszPath);
		return val;
	}
	else
	{
...
void CNumSpinCtrl::SetSpin(int iNumPlaces,double dLowerVal,double dUpperVal,double dDelta)//zll 2013.6.7 注意传入参数的类型 
{
	//设置旋转按钮绑定控件的小数点后面的位数
	SetDecimalPlaces (iNumPlaces);
	SetTrimTrailingZeros (FALSE);
	//设置旋转按钮的范围，及增量刻度
	SetRangeAndDelta (dLowerVal, dUpperVal, dDelta);
	//设置旋转按钮的当前刻度
	SetPos (dLowerVal);
}